...
# --- the opening theme, played over the poster. Matched by name so it is never
#     confused with the game's track.
if (-not $Opening) {
  $openFile = Get-ChildItem -Path $PSScriptRoot -File -ErrorAction SilentlyContinue |
              Where-Object { $_.BaseName -match '(?i)moonlight|ancient bridge' -and
                             $_.Extension -match '(?i)\.(mp3|m4a|ogg|opus|wav|aac|flac)$' } |
              Sort-Object Length                     # prefer the shortened cut
  if ($openFile) { $Opening = $openFile[0].FullName }
}
if ($Opening -and (Test-Path $Opening)) {
  $om = [regex]::Match($page, 'var POSTER_MUSIC_SRC = "[^"]*";')
  if (-not $om.Success) { throw "could not find the POSTER_MUSIC_SRC line in index.html" }
  $of = Get-Item $Opening
  $page = $page.Replace($om.Value, ('var POSTER_MUSIC_SRC = "' + (Get-DataUri $of.FullName) + '";'))
  Write-Host ("opening    : " + $of.Name + "  (" + [math]::Round($of.Length/1MB,2) + " MB)")
} else {
  Write-Host "opening    : none found - the poster will be silent"
}
